 ...
 */
public class EwriteDrawView extends View implements EwriteEditDrawDialog.EditDialogClickCallback, EwriteChooseColorDialog.ChangeColorCallBack {
    ...
    private int mPaintWidth = 3;    //真实画笔的宽度
    ...
    public EwriteDrawView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void pen_start(EwritePoint point, int msgWhat) {
        mPath = new Path();
        int x = EwritePointUtils.getX(point.getPointsStr(), SCALE_X);//X坐标
        int y = EwritePointUtils.getY(point.getPointsStr(), SCALE_Y);//Y坐标
        mPath.moveTo(x, y);
        mX = x;
        mY = y;
    ...
    private void removeEraserPointList(int x, int y) {
        for (int i = 0; i < mPointList.size(); i++) {
            ...
            int xPoint = EwritePointUtils.getX(point.getPointsStr(), SCALE_X);//X坐标
            int yPoint = EwritePointUtils.getY(point.getPointsStr(), SCALE_Y);//Y坐标
            //判断橡皮擦擦除位置是否有字
    ...
    public void dealData(String data) {
        flashdata = data.split("CA9");
        if (lastdata != null) {
            flashdata[0] = lastdata + flashdata[0];
            lastdata = null;
        }
        if (flashdata[flashdata.length - 1].length() < 40) {
            lastdata = flashdata[flashdata.length - 1];
        }
        for (int i = 0; i < flashdata.length; i++) {
            data = "CA9" + flashdata[i];
            if (data.length() == 40 && data.startsWith("CA9")) {
                try {
                    int V = Integer.parseInt(flashdata[i].substring(1, 3), 16); //错误帧标识
                    String F = flashdata[i].substring(0, 1);    //落笔书写抬笔标识
                    String B = flashdata[i].substring(5, 7);    //笔画号
                    String xStr = flashdata[i].substring(25, 27) + flashdata[i].substring(27, 29);  //X坐标
                    String yStr = flashdata[i].substring(29, 31) + flashdata[i].substring(31, 33);  //Y坐标
                    String pointStr = F + "_" + B + "_" + xStr + "_" + yStr;
     ...
     * 选择框移动时改变选择框中的点坐标
     */
    private void moveRectPointList(float xM, float yM, boolean isPaste) {
        if (mMoveRectPointList.size() > 0) {
            int pasteX = (int) (xM * SCALE_X - EwritePointUtils.getBeforeX(mMoveRectPointList.get(0).getPointsStr()));//X坐标
            int pasteY = (int) (yM * SCALE_Y - EwritePointUtils.getBeforeY(mMoveRectPointList.get(0).getPointsStr()));//Y坐标
            for (int i = 0; i < mMoveRectPointList.size(); i++) {
                EwritePoint point = mMoveRectPointList.get(i);
                int x;
                int y;
                if (isPaste) {
                    x = EwritePointUtils.getBeforeX(point.getPointsStr()) + pasteX;
                    y = EwritePointUtils.getBeforeY(point.getPointsStr()) + pasteY;
                } else {
                    x = EwritePointUtils.getBeforeX(point.getPointsStr()) + (int) (xM * SCALE_X);//X坐标
                    y = EwritePointUtils.getBeforeY(point.getPointsStr()) + (int) (yM * SCALE_Y);//Y坐标
                }
                String xStr = "";
                String yStr = "";
                if (x < 0) {
    ...
    public class MeaasgeHander extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (numberlast != null) {
                int F = EwritePointUtils.getF(numberlast);//落笔书写抬笔标识
                String B = EwritePointUtils.getB(numberlast);//笔画号
                //解决笔画漂移问题，当笔画号和上一笔不一致，则认为是落笔帧
                if (!last.equals(B)) {
                    F = 10;
                }
                last = B;
        ...
        switch (event.getAction()) {
            ...
            case ACTION_MOVE:
                ...
                if (!canEdit && !isEraser && EwriteCommenUtil.isLongPressed(startEditPointX, startEditPointY, moveEditPointX,
                    ...
                    createNoteEditDialog.showEditPopupWindow(startEditPointX, startEditPointY + EwriteCommenUtil.dip2px(getContext(), 0), true);
                    return true;
                }
                if (isEraser) {
...
//                        return true;
                    ...
                    addMoveCircle();
                    moveRectPointList(slideMovePointX, slideMovePointY, false);
...
//            mBitmap = Bitmap.createBitmap(a, b, Bitmap.Config.ARGB_8888);
    ...
    //设置默认画笔颜色
    private void setDefaultColor(String defaultColor) {
        this.defaultColor = defaultColor;
        mPaint.setColor(Color.parseColor(this.defaultColor));
    }

    //设置默认画笔宽度
    private void setmPaintWidth(int mPaintWidth) {
    ...
    public void paste() {
        ...
        if (EwriteSPUtils.get(this.getContext(), COPY_POINTS, "") != null && EwriteSPUtils.get(this.getContext(), COPY_POINTS, "").toString().length() > 0) {
            ...
            moveRectPointList(startEditPointX, startEditPointY, true);
            loopPointToOldPointList(mMoveRectPointList);
            //粘贴
            editPoint = new ArrayList<>();
            stepTypeList.add(PASTE_STEP);
